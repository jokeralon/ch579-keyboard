...
uint8_t keyboard_mode_rgb_flag = USB_MODE;
uint8_t keyboard_mode_flag = USB_MODE;
uint8_t keyboard_stop_scnf_time = 0;

uint8_t fn_flag = 0;
uint8_t boot_flag = 0;

uint8_t Lshift_flag = 0;
uint8_t Lctrl_flag = 0;
uint8_t Lalt_flag = 0;
...
const unsigned char myKeyBoard_FN_ATValue[12] = {0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45};
...
uint8 keyBoard_SendValue(void)
{
		...
		for (col_scanf_num = 0; col_scanf_num < COL_NUM; col_scanf_num++)
		{
			if (Keyboard_temp[row_scanf_num][col_scanf_num] == 1)
			{
				if (fn_flag == 1) // FN
				{
					...
					else if (strcmp(Keyboard_Map_Name[row_scanf_num][col_scanf_num], "ESC") == 0) //FN ESC 波浪键
					{
						buf[i] = 0x35;
						i++;
					}
	...
	// for (uint8_t buf_num = 2; buf_num < i; buf_num++)
	// {
	// 	if (Lshift_flag == 1)			// Lshift + ESC 替换为波浪键
	// 	{
	// 		if (buf[buf_num] == 0x29) 	// Lshift + ESC 替换为波浪键
	// 		{
	// 			buf[buf_num] = 0x32;
	// 			buf[0] &= ~(0x01 << 1); // 清除 Lshift 键值
	// 		}
	// 	}
	// }
